...
public class AsyncTaskUtil extends AsyncTask<String, Double, Boolean> {
    ...
    private String getFolderPath() {
        return Environment.getExternalStorageDirectory().toString() + "/楚咻/";
    }
    ...
    protected void onPostExecute(Boolean result) {
        ...
        if (result) {
            ...
            File file = new File(
                    Environment.getExternalStorageDirectory().toString() + "/楚咻/"
                    , app_name);
            Intent intent = new Intent(Intent.ACTION_VIEW);
            intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
            if (Build.VERSION.SDK_INT >= 24) {
                Uri apkUri = FileProvider.getUriForFile(mContext, "com.yangs.medicine.fileprovider", file);
